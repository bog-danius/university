...
const btnANames = document.getElementById("btnANames");
...
async function initSorting() {
    ...
    btnANames.addEventListener("click", async () => {
        const products = await fetchAllItems();
        const aProducts = products.filter(product =>
            product.name.trim().startsWith('П'));
        renderCatalog(aProducts);
    });
}
